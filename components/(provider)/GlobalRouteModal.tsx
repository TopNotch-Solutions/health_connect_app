...
export default function GlobalRouteModal() {
  ...
  if (!user || !user.userId || !activeRoute) {
    console.warn("GlobalRouteModal: Missing required data", {
      user: !!user,
      userId: user?.userId,
  ...
  console.log(
    "GlobalRouteModal - Patient profileImage:",
    activeRoute.patientId?.profileImage,
  );
  console.log(
    "GlobalRouteModal - ActiveRoute:",
  ...
  return (
    <ProviderRouteModal
      ...
      patientAddress={
        activeRoute.address?.route || activeRoute.address?.locality || undefined
      }
