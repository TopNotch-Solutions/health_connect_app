...
const HistoryCard = ({ item }: { item: typeof appointmentHistory[0] }) => (
    <View className="bg-white p-4 rounded-xl border border-gray-200 flex-1">
        ...
        <Text className={`text-sm font-bold ${item.status === 'Completed' ? 'text-secondary' : 'text-primary'}`}>{item.status}</Text>
      ...
      <View className="flex-1">
        <ScrollView contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
          ...
          <View className="px-6 mt-8">
            <Text className="text-3xl font-bold text-text-main mb-4">What do you need help with today?</Text>
          ...
          <View className="mt-8 px-6">
            <View className="flex-row justify-between items-center mb-3">
                 ...
                 <TouchableOpacity><Text className="font-semibold text-primary">See all</Text></TouchableOpacity>
            </View>
            <View className="flex-row" style={{ gap: 16 }}>
                {appointmentHistory.map(item => <HistoryCard key={item.id} item={item}/>)}
            </View>
          </View>
