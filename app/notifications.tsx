...
export default function NotificationsScreen() {
  ...
  const fetchNotifications = useCallback(
    async (isRefresh = false, page = 1) => {
      ...
      try {
        console.log(
          `Fetching notifications for userId: ${user.userId}, page: ${page}`,
        );
        // Backend extracts userId from JWT token
        const response = await apiClient.get(
          `/app/notification/all-user-notification?page=${page}&limit=10`,
        );
    ...
    async (notificationId: string) => {
      try {
        await apiClient.delete(
          `/app/notification/delete-notification/${notificationId}`,
        );
