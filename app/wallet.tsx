  ...
  const renderTransactionItem = ({ item }: { item: Transaction }) => (
    <View className="flex-row items-center justify-between p-4 border-b border-gray-200">
      <View className="flex-1">
        <Text
          className={`text-base font-semibold ${getTransactionTypeColor(item.type)}`}
        >
      ...
      <View className="items-end ml-4">
        <Text
          className={`text-base font-bold ${getTransactionTypeColor(item.type)}`}
        >
          {item.type === "withdrawal" || item.type === "transfer" ? "-" : "+"}P
          {item.amount.toFixed(2)}
        </Text>
        <View
          className={`${getStatusBadgeColor(item.status)} rounded-full px-3 py-1 mt-1`}
        >
          <Text
            className={`text-xs font-semibold capitalize ${getStatusTextColor(item.status)}`}
          >
            {item.status}
          </Text>
        </View>
          ...
          {isLoading ? (
            ...
            <View className="flex-1 items-center justify-center px-6">
              ...
              <Text className="text-gray-500 text-center mt-2">
                Your wallet transactions will appear here once you make your
                first transaction.
              </Text>
            </View>
          )}
